...
- **Any other slide edit** — read the `slide-authoring` skill before writing. It is the technical reference for everything inside `slides/<id>/`: file contract, the canvas (size comes from `canvas` in `open-slide.config.ts`, and portrait canvases change the layout rules), type scale, palette, layout, assets, PDF export, self-review checklist, and anti-patterns. `create-slide` and `apply-comments` both defer to it for the *how*.
