...
/** How far a page's content reaches, in canvas pixels. */
export type ContentExtent = {
  width: number;
  height: number;
};

/** How far content spills past the canvas, in canvas pixels. */
...
/**
 * Compare a page's content extent against the canvas.
 *
 * @param extent - Reach of the page's text, from {@link contentExtent}.
 * @param canvasWidth - Configured canvas width, in CSS pixels.
 * @param canvasHeight - Configured canvas height, in CSS pixels.
 * @returns Overflow per axis, or null when the page fits.
 */
...
/**
 * Build the console warning for a clipped page.
 *
 * @param where - Human-readable page identity, e.g. `my-deck page 3`.
 * @param overflow - Result of {@link measureOverflow}.
 * @param canvasWidth - Configured canvas width, in CSS pixels.
 * @param canvasHeight - Configured canvas height, in CSS pixels.
 */
...
/**
 * Whether an element holds a text node of its own, rather than only wrapping
 * other elements.
 *
 * @param el - Candidate element inside the page.
 */
...
/**
 * Warn once if a page's text is clipped by the canvas.
 *
 * @param el - The canvas element wrapping the rendered page.
 * @param where - Human-readable page identity used in the message.
 * @param canvasWidth - Configured canvas width, in CSS pixels.
 * @param canvasHeight - Configured canvas height, in CSS pixels.
 * @param key - Dedupe key; defaults to `where`.
 */
...
/** Clear the dedupe set. Exported for tests and for a full deck reload. */
